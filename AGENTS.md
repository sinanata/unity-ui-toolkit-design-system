...
4. **The showcase is the test suite.** Every component, state, and variant must appear in `Assets/DesignSystem/Resources/UI/Styles/DesignSystem/DesignSystemShowcase.uxml`. A rule change that does not update the showcase is incomplete.
...
6. You do not wire the runtime. `DesignSystemRuntime` auto-attaches to every UIDocument (and every PanelRenderer on 6000.5+) and injects toggle knobs, drives spinner rotation, animates skeleton shimmer, and wires drag and drop. If you clone templates lazily and want to avoid a one-frame flat-toggle flash, call the runtime's `EnsureToggleKnobs(root)` helper after the clone (see `docs/ARCHITECTURE.md`).

## Repository layout

- `Assets/DesignSystem/` is the shippable package, and the only folder a consumer copies.
  - `Resources/UI/Styles/DesignSystem/`: 14 USS files plus the showcase UXML. `DesignSystem.uss` is the master that `@import`s the rest in a load-bearing order.
  - `Resources/Textures/Icons/`: 63 white-fill SVGs.
  - `Runtime/`: `DesignSystemRuntimeBase<T>` plus `UIDocumentRuntime/` and `PanelRendererRuntime/` backends.
